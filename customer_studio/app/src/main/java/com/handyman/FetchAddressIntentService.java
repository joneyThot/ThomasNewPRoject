    ...
    private void deliverResultToReceiver(int resultCode, String message, Address address) {
        try {
            ...
            if (!Utils.validateString(message)) {
                String addrs = "";
                if (Utils.validateString(address.getAddressLine(0)))
                    addrs = address.getAddressLine(0);
                if (Utils.validateString(address.getAddressLine(1)))
                    if (!addrs.contains(address.getAddressLine(1)))
                        addrs = addrs + ", " + address.getAddressLine(1);
                if (Utils.validateString(address.getAddressLine(2)))
                    if (!addrs.contains(address.getAddressLine(2)))
                        addrs = addrs + ", " + address.getAddressLine(2);
                if (Utils.validateString(address.getAddressLine(3)))
                    if (!addrs.contains(address.getAddressLine(3)))
                        addrs = addrs + ", " + address.getAddressLine(3);
                bundle.putString(AppUtils.LocationConstants.LOCATION_DATA_STREET, addrs);
            }
...
//            bundle.putString(AppUtils.LocationConstants.LOCATION_DATA_LATITUDE, String.valueOf(address.getLatitude()));
//            bundle.putString(AppUtils.LocationConstants.LOCATION_DATA_LONGITUDE, String.valueOf(address.getLongitude()));
